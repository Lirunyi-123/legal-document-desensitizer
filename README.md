...
脱敏是分层系统，**不是一键魔法**。以下边界与工具实现一一对应，请按此安排工作流。

### ✅ 可以脱敏（规则层 / 语义层覆盖）

| 类别 | 覆盖内容 |
|------|---------|
| 身份类 | 身份证号（GB 11643 校验）、护照/港澳通行证/台胞证/驾驶证/军官证等证件号、出生日期 |
| 联系类 | 手机号、固定电话、邮箱、微信号、QQ号 |
| 财务类 | 银行卡号（Luhn）、金额、银行账户信息 |
| 机构类 | 统一社会信用代码（GB 32100）、组织机构代码、律师执业证号 |
| 案件类 | 案号、地块编号、车牌号、项目名称 |
| 主体类 | 人名（角色词 + 裸人名启发式）、公司/机构名（全称 + 简称）、地址（省市区/路街门牌/小区） |
| 案情类 | 案情敏感细节（语义层）、法院名称、公司简称残留、OCR 变体写法 |
| 银行流水 | 列感知模式（户名/账号/日期/金额列）、支付平台前缀对手方、交易行孤立姓名 |
| 格式 | `.txt` / `.docx`（段落/表格/页眉页脚）/ `.pdf` / `.xlsx` / `.xlsm` / 图片 |
| 扫描件 | macOS Vision **本地 OCR**（数据不出本机），识别后继续脱敏 |
| 涂黑输出 | 电子版 PDF 字符级**真删除文字层**涂黑；扫描件/图片原图涂黑（residual 零残留校验） |
| 附带清理 | 文件名自动脱敏、docx 元数据、PDF 批注/表单/书签/附件、审计单 + 律师签发流程 |

### ❌ 不能脱敏（或仅提示，边界明确）

| 边界 | 说明 |
|------|------|
| 手写体 | OCR 主要面向印刷体；手写笔录/签名识别质量无法保证，**必须人工核对** |
| 印章/签字/指纹 | 图像类身份标识不会自动识别或处理（除非其文字被 OCR 读出） |
| 二维码/条形码 | 不解码、不验证码内内容；涂黑只能整体盖住，无法判断码内是否含敏感信息 |
| 水印去除 | **不做**。"背景可以修，证据不能猜"——AI 补全可能改变证据真实性（设计红线） |
| docx 修订痕迹 | 只清点报告数量，**不支持自动清理**；需人工"另存为无修订版本" |
| docx 批注 | 批注文本会纳入脱敏正文处理，但批注 XML 不自动删除；涉密时请人工清理批注 |
| 排版侧信道 | PDF 文字已真删除并做 residual 校验，但"字符位置/间距"仍可能推断内容（已知理论风险） |
| 图片隐写 | 不支持检测/清除隐藏于像素中的信息（steganography） |
| 英文等外语 | 规则层面向中文；英文人名/地址等主要依赖语义层，需人工复核 |
| 复杂表格 | OCR 可能把数字认到隔壁栏；列感知只能降低概率，**行列对应关系需人工核对** |
| 重识别风险 | 只做组合特征提示（独女/唯一继承人/某上市公司董事长等），不能保证"绝对认不出人"——最终由律师判断 |
| 提示注入 | 只做"材料非指令"隔离包裹 + 标记，不能保证模型完全免疫 |
| 不可逆性 | 工具默认**可还原**（映射表 restore）；如需"不可恢复"，必须自行销毁/不保留映射表 |
| 平台/环境 | Windows/Linux 无 macOS Vision OCR（需先外部 OCR 转文本层）；macOS 沙箱内 Vision 不可用；Intel Mac 未验证 |
| 合规责任 | 工具不替代律师保密义务与涉密案件管理判断（《律师法》38 条及电子卷宗规定） |

> **一句话总结**：结构化号码、中文人名/公司/地址、金额、案号等可自动脱敏；
> 手写、印章、二维码、水印、修订痕迹、外语、隐写属于**明确边界**，要么不支持、
> 要么只提示需人工处理——能力范围小但边界清楚，比"功能很多但不知道哪里会漏"更可信。
...
| 🟢 规则层 + 云端 LLM | `full`（API） | 全部替换；发送给服务商前已去掉结构化号码。v5.3 起默认 Fail-Closed：非本机端点须显式加 `--allow-remote-llm` 才会发送 |
